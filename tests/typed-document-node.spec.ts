...
describe("TypedDocumentNode", () => {
  ...
  it("Should not output object literal", async () => {
    ...
    expect(
      findAncestor(
        node,
        (n) => ts.SyntaxKind[n.kind] === "ObjectLiteralExpression"
      )
    ).toBeFalsy();
  });
  ...
  it("Should handle custom query type", async () => {
    ...
    const res = (await plugin(
      ...
      [{ location: "", document: ast }],
      {
        documentTypeImportDirective:
          "my-custom-type-module#CustomDocumentNamedImport",
      },
      { outputFile: "" }
    )) as Types.ComplexPluginOutput;

    expect(res.prepend.length).toEqual(1);
    expect(res.prepend[0]).toEqual(
      "import { CustomDocumentNamedImport as DocumentNode } from 'my-custom-type-module';"
    );
  });
  ...
  it("Should work with fragment imports via fragmentImportsSourceMap", async () => {
    ...
    expect(res.prepend).toContain(
      `import { JobFragmentFragment } from './_fragment.graphql';`
    );
  });
  ...
  it("Should combine imports from same file into same import statement", async () => {
    ...
    expect(res.prepend).toContain(
      `import { JobFragmentFragment, OtherJobFragmentFragment } from './_fragment.graphql';`
    );
  });
  ...
  it("Should work with nested fragment imports via fragmentImportsSourceMap", async () => {
    ...
    expect(res.prepend).toContain(
      `import { JobFragmentFragment } from './_anther_fragment.graphql';`
    );
  });
  ...
  it("Should also produce exported query", async () => {
    ...
    expect(
      ((typeDec.type as ts.TypeReferenceNode).typeName as ts.Identifier)
        .escapedText
    ).toEqual("DocumentNode");
